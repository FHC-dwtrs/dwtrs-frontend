...
export default function GroupPage({ page, setPage }: Props) {
  ...
  // ---------------------------------------------------------
  // LOAD CASES
  // Backend already returns cases assigned to the
  // currently logged-in user's organizational unit.
  // ---------------------------------------------------------
  ...
  //
  // IMPORTANT:
  // Do NOT filter by "Group A1".
  //
  // GET /cases already filters by the logged-in user's
  // current organizational unit on the backend.
  // ---------------------------------------------------------
  ...
  // All cases assigned to this Group before UI filtering.
  const allGroupCases = cases.map(mapCaseToRecord)
...
/>
  ...
  if (page === 'remarks') {
    const allRemarks = allGroupCases.flatMap(c =>
      c.remarks.map(r => ({
        ...r,
        caseId: c.id,
        subject: c.subject,
        ...
        {loadingCases ? (
          ...
          <div className="space-y-3">
            {allRemarks.length === 0 ? (
              <EmptyState
                icon="💬"
                title={t('empty_noRemarks')}
                sub={t('empty_noRemarksDesc')}
              />
            ) : (
              allRemarks.map((r, i) => (
                <div
  ...
  // DELAYED CASES
  //
  // Backend does not have a "Delayed" status.
  // We calculate delayed based on how long the case has been
  // active.
  // ---------------------------------------------------------

  if (page === 'delayed') {
    const delayed = allGroupCases.filter(c => {
      const originalCase = cases.find(
      ...
      const diffDays = Math.ceil(
        (Date.now() -
          new Date(originalCase.submittedAt).getTime()) /
          (1000 * 60 * 60 * 24)
      )
    ...
    return (
      <div className="p-6 space-y-4">
        ...
        {loadingCases ? (
          ...
          <EmptyState
            icon="✅"
            title={t('empty_noDelayed')}
            sub={t('empty_allOnTrack')}
          />
        ) : (
          delayed.map(c => {
            const originalCase = cases.find(
            ...
            const diffDays = originalCase
              ? Math.ceil(
                  (Date.now() -
                    new Date(
                      originalCase.submittedAt
                    ).getTime()) /
                    (1000 * 60 * 60 * 24)
                )
              : 0
            ...
            return (
              ...
              >
                ...
                <p className="text-xs text-red-600 mt-1">
                  ⚠ {diffDays} days in current stage
                </p>
              </div>
            )
          })
  ...
  if (page === 'dashboard') {
    ...
    //
    // mapCaseToRecord converts backend statuses such as:
    // SUBMITTED -> New
    // IN_PROGRESS -> In Progress
    // SENT_BACK_FOR_CORRECTION -> Returned
    //
    const needsAction = allGroupCases.filter(c =>
      ['New', 'Returned', 'In Progress'].includes(c.status)
    )

    // Backend statuses used for real KPI counts.
    const newCases = cases.filter(
      c => c.status === 'SUBMITTED'
    ).length

    const pendingCases = cases.filter(
      c => c.status === 'PENDING_CLARIFICATION'
    ).length

    const returnedCases = cases.filter(
      c => c.status === 'SENT_BACK_FOR_CORRECTION'
    ).length
...
// Small helper so the JSX stays readable.
function filteredCasesEmpty(
  groupCases: CaseRecord[]
): boolean {
  return groupCases.length === 0
}